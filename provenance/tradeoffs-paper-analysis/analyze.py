...
DISPLAY_NAMES={
 ...
 'header_commit':'Header commitments','encode_commit_total':'Encoding and commitments',
 'header_parse_verify':'Header decoding and verification','derive_source':'Source commitment lookup',
...
def write_tex(path,env,manifests,summary,raw_name):
    ...
    table('Light-client computation. Payload counts exclude query indices and a 32-byte context identifier. Both schemes have the same source-compressed header.','lrlrr',body,'tab:measured-client')
...
def figures(summary,folder):
    ...
    ax.set_xticks(range(len(ops)),['Outer mix','Inner evaluation','Header commit','Measured total']);ax.set_ylabel('Time (ms)');ax.legend();fig.tight_layout()
