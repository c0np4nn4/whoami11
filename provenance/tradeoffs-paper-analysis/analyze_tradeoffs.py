...
def write_tex(path,env,manifests,summary,dataset):
    ...
    lines=[
        ...
        f'and are capped at {cfg["max_iterations"]}. Header production, complete encoding, supplied auxiliary '
    ...
    lines.extend([
        ...
        r'after each fixed header is received. This is not a Fiat--Shamir challenge. Nonzero residual blocks are '
        ...
        r'generation is outside client timing. Cold clients parse and validate their header, start with an empty '
        r'group cache, derive or authenticate each touched group once, and parse and verify every response. '
        r'Warm clients reuse verified header/group state for the same object but still parse and verify every '
        r'response. There is no proof-result cache. Defensive point-validity rechecks in public APIs are included. '
        r'The supplied header alone does not establish global membership: group authentication enforces both pairing equations.',
    ...
    ops=[('Header production','variant_header_commit','public_srs'),
         ('Encode and publish commitments','variant_encode_total','public_parameters'),
         ('Header parsing and verification','variant_header_verify','cold'),
    ...
    lines.append('The paired LR Profile~B/Profile~A header-production ratio is '+
    ...
    table('Commitment constructions for the same LR-DAS code (ms). Group rows use the same parity group '
          'and received positions. A new peer has resident public parameters but no cached header/group state.',
    ...
    body=[]
    for title,field in [('Header','header_bytes'),('Per-group auxiliary data','auxiliary_bytes_per_group'),
    ...
    body.extend([f'Local polynomial coefficients & {r*32} & {r*32} & {r*32}',
                 'Fresh-peer response, header excluded & 80 & 80 & 224'])
    ...
    lines.append(
        ...
        r'The smaller supplied header benefits this access pattern, while its auxiliary-state requirement '
    ...
    lines.extend([
        ...
        r'from the common header, without per-group auxiliary data. A peer with a previously authenticated group '
    ...
    ops=[('Restore complete message and codeword','diagonal_full_decode','accepted_values'),
         ('Restore and authenticate against header','diagonal_decode_authenticate','accepted_values'),
    ...
    lines.extend([
        ...
        r'under every erasure pattern. Supplied commitments have different header and auxiliary-state costs. '
