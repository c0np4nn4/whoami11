...
fn run(config: Config, out: &Path, process_run_id: usize, filter: String) -> Result<()> {
    ...
    if !["", "tradeoffs", "separation"].contains(&config.suite.as_str()) {
    ...
    let mut meta = vec![];
    let result = (|| {
        if config.suite == "separation" {
            return separation::run(&mut runner, &domain, &srs, &mut meta);
        }
...
fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    if args.is_empty() || args.iter().any(|s| s == "--help") {
        println!("Paper tables: bench --table 2|3|all [--config reference.json] [--out NEW_DIRECTORY] [--runs 3]\n\nUsage: bench --config FILE --out NEW_RUN_DIRECTORY [--run N] [--filter OP_SUBSTRING] [--seed N]\nEach output run is create-new. No existing samples are overwritten.");
        return Ok(());
    }
